...
export function BusinessInfoProvider({ children }) {
  ...
  // Obtener horarios como array ordenado
  const getScheduleArray = () => {
    const s = info.schedule || {};
    return [s.weekdays, s.saturday, s.sunday, s.emergency].filter(Boolean);
  };

  // Obtener redes sociales con nombre e íconos SVG
  ...
  return (
    <BusinessInfoContext.Provider
      value={{ info, loading, getWhatsAppUrl, getPhoneDisplay, getScheduleArray, getSocialLinks }}
    >
      {children}
    </BusinessInfoContext.Provider>
  );
...
export function useBusinessInfo() {
  ...
  if (!ctx) throw new Error('useBusinessInfo debe usarse dentro de BusinessInfoProvider');
  return ctx;
}